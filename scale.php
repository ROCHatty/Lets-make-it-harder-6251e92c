...
function createPossibilities($nums) {
	$possibilities = array();
	foreach ($nums as $num) {
		array_push($possibilities, array("num"=>$num, "text"=>$num));
		for ($i=0;$i<count($nums);$i++) {
			$count = 1;
			while ($i+$count < count($nums)) {
				array_push($possibilities, array("num"=>$num+$nums[$i+$count], "text"=>$num." and ".$nums[$i+$count]));
				$c = 1;
				while ($i+$c < count($nums)) {
					array_push($possibilities, array("num"=>$num+$nums[$i+$count]+$nums[$i+$c], "text"=>$num.", ".$nums[$i+$count]." and ".$nums[$i+$c]));
					$c++;
				}
				$count++;
			}
